...
// User status tracking with initial values
let userStatuses = {
  '1': { 
    isOnline: true, 
    lastSeen: new Date(), 
  ...
  '2': { 
    ...
    lastSwipedAt: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(), 
    hasMatches: false 
  },
  '3': { 
    isOnline: false, 
    lastSeen: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), 
    lastSwipedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), 
    hasMatches: true 
  }
};
...
// Endpoint to record a swiped restaurant
app.post('/api/users/:userId/swiped', (req, res) => {
  const { userId } = req.params;
  const { restaurantId, restaurantName, direction } = req.body;
  
  console.log(`Recording ${direction} swipe for user ${userId}, restaurant ${restaurantName}`);
  
  // Add to swipe history with expiration (24 hours)
  const swipe = {
    userId,
    restaurantId,
    restaurantName,
  ...
  if (userStatuses[userId]) {
    userStatuses[userId].lastSwipedAt = new Date().toISOString();
  }
  
  res.status(201).json(swipe);
});

// Endpoint to check if a restaurant has been swiped
app.get('/api/users/:userId/swiped/:restaurantId', (req, res) => {
  const { userId, restaurantId } = req.params;
  
  const now = new Date().toISOString();
  const isSwiped = swipeHistory.some(
...
// Endpoint to clear swipe history for a user
app.delete('/api/users/:userId/swiped', (req, res) => {
  const { userId } = req.params;
  
  const initialLength = swipeHistory.length;
  swipeHistory = swipeHistory.filter(swipe => swipe.userId !== userId);
  
  const removedCount = initialLength - swipeHistory.length;
  console.log(`Cleared ${removedCount} swipes for user ${userId}`);
  
  res.json({ 
    success: true, 
    message: `Cleared ${removedCount} swipes`,
    count: removedCount
  });
});
...
app.put('/api/users/:userId/status', (req, res) => {
  ...
  console.log(`Updated status for user ${userId}:`, userStatuses[userId]);
...
app.post('/api/users/status', (req, res) => {
  ...
  const statusData = userIds.map(userId => {
...
app.delete('/api/notifications/user/:userId/clear-all', (req, res) => {
  const { userId } = req.params;
...
app.post('/api/notifications/test/match', (req, res) => {
...
app.post('/api/notifications/test/message', (req, res) => {
...
app.post('/api/notifications/test/invitation', (req, res) => {
